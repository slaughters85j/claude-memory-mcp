...
const argv = minimist(process.argv.slice(2), { boolean: "overwrite" });
...
async function generateContentOverview(rawDocs, model) {
...
async function catalogRecordExists(catalogTable, hash) {
    const query = catalogTable.query().where(`hash = ${sqlString(hash)}`).limit(1);
    const results = await query.toArray();
    return results.length > 0;
}
const directoryLoader = new DirectoryLoader(filesDir, {
...
async function processDocuments(rawDocs, catalogTable, skipExistsCheck) {
    // group rawDocs by source for further processing
    const docsBySource = rawDocs.reduce((acc, doc) => {
...
async function seed() {
    ...
    const catalogStore = catalogRecords.length > 0 ?
        await LanceDB.fromDocuments(catalogRecords, new OllamaEmbeddings({ model: defaults.EMBEDDING_MODEL }), { mode: overwrite ? "overwrite" : undefined, uri: databaseDir, tableName: defaults.CATALOG_TABLE_NAME }) :
    ...
    const filteredRawDocs = rawDocs.filter((doc) => !skipSources.includes(doc.metadata.source));
    ...
    const vectorStore = docs.length > 0 ?
        await LanceDB.fromDocuments(docs, new OllamaEmbeddings({ model: defaults.EMBEDDING_MODEL }), { mode: overwrite ? "overwrite" : undefined, uri: databaseDir, tableName: defaults.CHUNKS_TABLE_NAME }) :
