...
export function isCommitConflict(error: unknown): boolean {
...
export async function withCommitRetry<T>(
  op: () => Promise<T>,
  refresh: () => Promise<void> = refreshTables,
): Promise<T> {
 ...
 */
export async function safeTouchTopicLastReferenced(id: string): Promise<void> {
  try {
    await touchTopicLastReferenced(id);
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    console.error(
      `warning: could not touch last_referenced_at for topic ${id} ` +
