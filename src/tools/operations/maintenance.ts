...
export class PruneStaleDataTool extends BaseTool<PruneStaleDataParams> {
  name = "prune_stale_data";
  description =
  ...
  async execute(params: PruneStaleDataParams): Promise<ToolResponse> {
    try {
      ...
      const memoriesToPrune = allMemories.filter(
        (m) =>
          m.updated_at < memoryCutoffISO &&
      ...
      const todosToPrune = completedTodos.filter(
        (t) => t.completed_at && t.completed_at < todoCutoffISO
      );
...
export class ExportTopicTool extends BaseTool<ExportTopicParams> {
  name = "export_topic";
  description =
  ...
  async execute(params: ExportTopicParams): Promise<ToolResponse> {
    try {
      ...
      const todos = await listTodos({
        topic_id: params.topic_id,
        status_filter: todoStatusFilter as any,
        limit: 500,
      });
      ...
      if (format === "json") {
        ...
        if (todos.length > 0) {
          ...
          const closedTodos = todos.filter(
